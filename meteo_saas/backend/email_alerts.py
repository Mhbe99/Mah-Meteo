...
def envoyer_push_notification(
    ...
    # Désérialise la clé privée VAPID (format DER base64url) en objet Vapid01.
    # webpush() n'accepte qu'une string ou une instance Vapid/Vapid01 — jamais
    # un objet clé cryptography brut, d'où le passage explicite par Vapid01.
    try:
        from py_vapid import Vapid01
        vapid_key_obj = Vapid01.from_string(vapid_private)
        print("[PUSH] Clé VAPID désérialisée OK")
...
def _check_push_cooldown(key: str) -> bool:
    """Retourne True si on peut envoyer un push, False si cooldown push actif."""
    last = _last_push_sent.get(key)
    if last and (datetime.now() - last).total_seconds() < _PUSH_COOLDOWN_SECONDS:
        return False
    _last_push_sent[key] = datetime.now()
    return True
...
def send_pollution_alert(to_email: str, company_name: str, zones_alertes: list, state_file: str = "exports/last_pollution_alert.json", client_id: int | None = None) -> bool:
    """
    Envoie une notification push pollution (AQI >= 40) avec cooldown différencié:
    - 40-59: 6h
    - 60-79: 3h
    - 80+: 1h
    to_email/company_name conservés pour compatibilité de signature avec les appelants existants.
    """
    if not zones_alertes or client_id is None:
        return False
    ...
    if last:
        try:
            ...
            if delta < cooldown_seconds:
                print(f"[PUSH] Cooldown pollution {cooldown_key} actif ({int(delta)}s/{cooldown_seconds}s)")
                return False
        except Exception:
            pass
    ...
    try:
        ...
        nb_push = envoyer_push_notification(
            ...
            titre=titre,
            corps=corps,
            type_alerte="pollution",
            url="/?tab=1"
        )
        db_push.close()
    except Exception as e:
        print(f"[PUSH] Erreur envoi pollution: {e}")
        return False
    ...
    if sent:
        try:
            ...
            print(f"[PUSH] Alerte pollution envoyée mais sauvegarde cooldown impossible: {e}")
    return sent
...
def render_bulletin_email_html(to_email: str, company_name: str, zones: list, incidents: list = None, creneau: str = "") -> tuple[str, str]:
    """Construit le sujet et le HTML du bulletin météo visuel.

    Cette fonction est partagée entre l'envoi réel et la génération de previews
...
def send_bulletin_email(to_email: str, company_name: str, zones: list, incidents: list = None, creneau: str = "") -> bool:
    ...
    """
    try:
        subject, html = render_bulletin_email_html(to_email, company_name, zones, incidents=incidents, creneau=creneau)
        sent = _envoyer_email(subject, html, _get_all_recipients(to_email), from_name="Mah Météo")
        return bool(sent)
    except Exception as exc:
        print(f"[BULLETIN] Exception send_bulletin_email: {exc}")
        return False
