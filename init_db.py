...
if existing_client:
    ...
    import os
    client = Client(
        username=os.getenv("INIT_CLIENT_USERNAME", "service-meteo"),
        password_hash="hashed_dummy_password",  # Dummy
        company_name=os.getenv("INIT_CLIENT_COMPANY", "GEODIS"),
        email=os.getenv("INIT_CLIENT_EMAIL", ""),
        plan=os.getenv("INIT_CLIENT_PLAN", "pro"),
        active=1
    )
    db.add(client)
    db.commit()
